...
def audit_dependencies(requirements_file: str = "requirements.txt") -> dict:
    ...
    result = subprocess.run(
        ["pip-audit", "-r", requirements_file, "--format=json", "--progress-spinner=off"],
        capture_output=True,
        text=True,
        timeout=180,
    )
    ...
    try:
        ...
        for dep in data:
            for vuln in dep.get("vulns", []):
                ...
                cves.append(f"{dep.get('name', '?')} {dep.get('version', '?')}: {vid} — {desc}")
    except (json.JSONDecodeError, TypeError):
        summary = (result.stdout + result.stderr).strip()[-300:]

    return {"cve_count": len(cves), "cves": cves, "summary": summary or f"{len(cves)} CVE(s) found"}
